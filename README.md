...
## Day 232 (2021/04/02)
今天早上学习一下函数式编程的思想：  
    + 一切皆函数，用函数组合的方式描述计算过程
    + 避免繁琐的内存管理
    + 函数的结果仅仅依赖于提供给它的参数
    + 将计算机运算看作是数学中函数的计算，避免了状态和变量的概念

函数式编程的特点：
    + 变量总是代表实际值，变量没有存储位置和左值的概念没有赋值操作，只有常量，参数，和值
    + 引用透明性，任何函数的值只取决于它的参数的值，而与求得参数值的前后或调用函数的路径无关
    + 函数可以不依赖任何其他的对象单独存在，可以将函数作为参数传入另一个函数，也可以作为函数的返回值
    + 合成和柯里化
    + 惰性求值与并行（先进入函数体，只有当需要实参值的时候才计算所需的实参值）
...
今天发现了两首非常好听的歌，一个是 Heavens Feel 剧场版第三部主题曲 春はゆく，Aimer 唱的，第二个是罪恶王冠的插曲。  
特别是第一首，已经出不来了。  
晚安。  
...
晚上去上函数式编程的实验。 
实验完回寝室后，刷点题然后做点 JLPT 真题，记下日语单词，然后睡觉了。  
晚安。  
...
从反汇编文件里面可以看到：  
···asm
jalr	1822(ra) # ffffffffc020c72a <_ZN4core3ptr8non_null16NonNull$LT$T$GT$4cast17he354a7601adfce42E>
···
类似于这种，它会跳转到一些不属于共享段的函数中去执行。最棘手的是，这种跳转不是一次性的，在跳转到的非共享段函数里面可能还会二次跳转到另外的非共享段函数。  
...
看到一半去下 vivado 了，顺便看番。发现了一步很好看的新番：《哥斯拉 奇点》。一口气看了 3 集，等后面更新。  
练下托福和日语听力，晚安。  
...
+ 桌面虚拟化：桌面虚拟化 != 操作系统虚拟化，后者允许您在单台机器上部署多个操作系统，而桌面虚拟化则允许中央管理员一次向数百台物理机部署模拟桌面环境
...
看了下 io_uring 的设计思想，发现上面这种思路就是参考 io_uring 的。。。被打脸了。  
...
晚上逛 github 看到一个日本人，写 Rust 代码和 涉及 riscv，我感到非常激动，没想到能找到一个和我方向如此相近的日本人。我马上通过推特联系他，目前还没回复。  
晚安。  
...
地址空间与进程：  
一对多 vs 多对一
资源占用和释放的单位，一个进程打开一个文件，第二个进程打开另外一个文件（访问另外一个进程的数据，安全性 vs 性能）
共同承担错误的单位，出现不可恢复的错误，关闭整个进程。  
...
课上金老师讲到多核并行，提到对于每个核的本地数据和远程数据。我对于一个核的本地数据和远程数据不是很理解，内存不都是挂在总线上的么，怎么会有本地和远程之分。  课后去问了金老师，老师说本地数据是指片内 Cache，远程数据是指片外的 Cache 或 内存或其他核的片内 Cache。  
这样一来我就理解了。  
...
中午的时候 wrj 学长给我写的 async-virtio-driver 发了一个 issue：https://github.com/HUST-OS/async-virtio-driver/issues/1  
...
+ Rc<T> 是不能 Send 的，因为如果克隆了 Rc<T> 的值并尝试克隆的所有权转移到另一个线程，这两个线程都可能同时更新引用计数。
...
下午的成果是这个 pr: https://github.com/HUST-OS/async-virtio-driver/pull/2  
...
回到寝室后继续写数据库实验。  
晚上写实验遇到一个问题，就是我想定义一个结构体，这个结构体包含多种类型的结构体的集合，类似 Vec<Box<dyn T>>，而且这个结构体需要是 Send 和 Sync 的。简单来说我是想要实现一个并发安全的支持多态的结构体。  
...
## Day 307 (2021/06/16)
今天基本写完了物联网的对象存储系统实验，并在 minio 系统上做了测试。已经整理好了文档推到了 [cs-course](https://github.com/cs-course)仓库。  
...
下午在修改 `async-virtio-driver`，使其正确区分`块`和`扇区`的概念。  
...
* 项目：
    * [飓风内核](https://github.com/HUST-OS/tornado-os): 基于共享调度器的协程内核，Rust 语言实现
    * [xv6-k210](https://github.com/HUST-OS/xv6-k210): 在勘智 K210 板子上移植 xv6-riscv 操作系统
    * [RustSBI](https://github.com/rustsbi/rustsbi): 用 Rust 语言实现 RISCV 指令集监管者模式二进制接口(一种运行时实现)
    * [rust-xs-evaluation](https://github.com/OpenXiangShan/rust-xs-evaluation): 为香山处理核仿真测试平台提供用 Rust 语言编写 benchmark 的支持(中科院计算所实习项目)
    * [async-virtio-driver](https://github.com/HUST-OS/async-virtio-driver): 基于 Rust 异步语法的 virtio 异步版前端块设备驱动
    * [bloom-filters](https://github.com/nervosnetwork/bloom-filters): 一个布隆过滤器的 Rust 语言实现
* 社区参与：
    * [HUST-OS](https://github.com/HUST-OS): 华中科技大学系统能力竞赛战队，维护者
    * [RustSBI](https://github.com/rustsbi): RustSBI 社区的维护者
    * [OpenXiangShan](https://github.com/OpenXiangShan): 香山处理核社区的开源贡献者
    * [rust-embedded](https://github.com/rust-embedded): Rust 嵌入式社区的开源贡献者
* 奖项：
...
* const 的用法
    ...
    * 一些例子：
        ...
        * `const int *a`，整形数不可改，指针可变
        * `int* const a`，整型数可变，指针不可变
        * `int const *a const`，整型数和指针均不可变
* volatile 的作用和用法
    * 一个定义为 volatile 的变量是说这个变量可能会被意向不到地改变，这样，编译器就不会去假设这个变量的值，而是每次读取这个变量的值的时候都会重新去内存中读取，而不是使用保存在寄存器里面的备份
    * 以下几种情况需要用到 volatile 变量
        * 并行设备的硬件寄存器，比如外设的状态寄存器
        * 一个中断服务子程序中会访问到的非自动变量
        * 多线程应用中被几个任务共享的变量
* const 常量和 #define 的区别
    * #define 只是文本替换，const 创建了一个只读变量
    * 在程序运行过程中 const 变量只有一个拷贝，而 #define 所定义的宏变量有多个拷贝，因此后者内存开销比较大
* 经典的 sizeof(struct) 和 sizeof(union) 内存对齐
    * 内存对齐作用
        * 某些硬件实现不支持非对齐地址访问
        * 访问非对齐的内存地址，处理器需要进行两次访存操作，对于对齐的内存地址，处理核仅需要一次
    * 结构体内存对齐的三大规则：
        * 对于结构体的各个成员，第一个成员的偏移量是 0，排列在后面的成员其当前偏移量必须是当前成员类型的整数倍
        * 结构体内所有数据成员各自内存对齐后，结构体本身还要进行一次内存对齐，保证整个结构体占用内存大小是结构体内最大数据成员的最小整数倍
        * 如程序中有 `#pragma pack(n)` 预编译指令，则所有成员对齐以 n 字节为准，不再考虑当前类型以及最大结构体内类型
    * 联合体内存对齐的两大规则
        * 找到占用字节数最多的成员
        * union 的字节数必须是占用字节数最多的成员类型的大小的倍数，并且需要能够容纳其他的成员
* 位域
    * C 语言允许在一个结构体中以位为单位来指定其成员所占内存长度，这种以位为单位的成员称为“位域”，利用位域能够用较少的位数存储数据。
    * 一个位域必须存储在同一个存储单元中，不能跨两个单元，如果第一个单元空不能容纳下一个位域，则该空间不用，而从下一个单元起存放该位域
    * 位域声明和结构体类似
    * 位域的成员必须是 int, unsigned int, signed int
* inline 函数
    * 在 C 语言中，如果一些函数被频繁调用，不断有函数入栈，即函数栈，会造成栈空间或栈内存的大量消耗，为了解决这个问题，特别引入了 inline 修饰符，表示为内联函数
    * 内联函数通常是将它在程序中的每个调用点上“内联地”展开
...
* 内存四区
    ...
    * 代码区(.text)
* 判断所使用机器大小端的方法
    * 联合体方法判断方法：利用联合体从低地址开始存，且同一时间只有一个成员占有内存的特性：
    ```C
    #include<stdio.h>
...
## Day 350 (2021/07/29)
继续准备大疆笔试：  
* 在有数据 Cache 的情况下，DMA 数据链路为：外设-DMA-DDR(内存)-cache-CPU
* UART 总线协议
* SPI 总线协议
* I2C 总线协议
* USB 总线协议
* TCP/UDP
    ...
    * UDP: 用户数据报协议，是一个简单的面向数据报的运输层协议，UDP 不提供可靠性，它只是把应用程序传给 IP 层的数据报发出去，并不保证它们可以到达
    * TCP 是面向连接的，UDP 是面向无连接的
    * TCP 是面向字节流的，UDP 是基于数据报的
    * TCP 保证数据正确性，UDP 可能丢包
    * TCP 保证数据顺序到达，UDP 不保证
* TCP 在传输数据之前，会有三次握手来建立连接，并且在数据传输时，有确认，窗口，重传，拥塞控制机制，在数据传完之后，还会断开来连接节约系统资源
* OSI 七层结构：应用层，表示层，会话层，传输层，网络层，数据链路层，物理层
* TCP/IP 四层结构：应用层，传输层，网络层，数据链路层
* GCC 编译优化：
    * O1：消耗些许编译时间，主要对代码的分支，常量以及表达式进行优化
    * O2：会尝试更多的寄存器级的优化以及指令集的优化，在编译器占用更多的内存和编译时间
    * O3：更近一步优化，例如使用伪寄存器网络，普通函数的内联，以及针对循环的更多优化
    * O4：主要是对代码大小的优化
* FIQ 的入口地址是 0x1c,IRQ 的入口地址是 0x18（？）
* PALIGN_DOWN(x,align)  (x & (- align))
* PALIGN_UP(x,align) ((x + align - 1) & (-align))
...
## Day 352 (2021/07/31)
今天尝试将异步 fat32 文件系统移进飓风内核中去，但遇到不少问题，目前还没解决。  
...
## Day 355 (2021/08/03)
这两天继续写内核，目前已经将文件系统移植到内核里面去了，并且稳定了 `event` 这个 crate，并利用其完美地解决了如何通知块设备驱动的问题。  
今天晚上已经实现了从文件系统中加载用户程序到内存，并且可以通过一个异步任务从内核进入用户态。  
晚安。  
...
## Day 357 (2021/08/05)
这两天的进度还是挺不错的，完成了以下部分：  
+ 不同地址空间之间的切换
+ 异步块设备读写系统调用

其中异步块设备读写系统调用这块还会有一些问题，但已经想到解决方案，打算放到后面作为一个模块来写。  
明天想一下进程怎么写，主要是要想清楚我们需要进程来干些什么事，进程的作用是什么。这块想清楚了，我们甚至可以不需要进程。  
晚安。  
...
## **一段旅途的结束**
23日，第一届全国大学生操作系统比赛落幕，获功能赛道团队二等奖，个人最佳技术支持奖。奖金一万元。

赛后被队友背叛，断绝朋友关系，大路朝天各走一边。

从此退隐Rust与OS社区，走向了工业界，开始新的征程。第一站是字节跳动。

从2020年暑假操作系统夏令营开始到现在的这一段旅途，尽管曲折，艰难，结果并不是那么美好，但仍是一段珍贵的回忆。
